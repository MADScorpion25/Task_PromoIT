...
public class CustomList<T> implements List<T>, AuthorHolder{
    ...
    public CustomList(int capacity, double increaseFactor){
        if(capacity <= 0){
    ...
    public List<T> subList(int inclusive, int exclusive) {
        ...
        CustomList<T> newList = new CustomList<>(exclusive - inclusive + 1);
    ...
    private class CustomIterator implements ListIterator<T>{
        ...
        public void add(T item) {
            CustomList.this.add(cursor, item);
            next();
        }
    }
}
